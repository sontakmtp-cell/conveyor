    ...
    def execute(self) -> CalculationResult:
        ...
        if self.p.V_mps is None or self.p.V_mps <= 0:
            try:
                ...
                surcharge_angle_deg = getattr(self.p, 'surcharge_angle_deg', 20.0) or 20.0
                ...
                self.r.belt_width_selected_mm = optimal_width
                ...
                self.r.speed_warnings = speed_warnings
                
                # Cập nhật V_mps trong parameters để sử dụng trong các tính toán tiếp theo
                self.p.V_mps = v_final
                
                print(f"DEBUG AUTO SPEED: Calculated v_final={v_final:.3f} m/s, optimal_width={optimal_width}mm")
                print(f"DEBUG AUTO SPEED: v_req={v_req:.3f} m/s, v_rec={v_rec:.3f} m/s, area={area_m2:.6f} m²")
                
                # Thêm warnings nếu có
                if speed_warnings:
                    self.r.warnings.extend(speed_warnings)
                    
            except Exception as e:
                print(f"DEBUG AUTO SPEED ERROR: Failed to calculate auto speed: {e}")
                # Fallback: sử dụng tốc độ mặc định an toàn
                self.p.V_mps = 2.0
                self.r.warnings.append(f"Không thể tính tốc độ tự động, sử dụng tốc độ mặc định {self.p.V_mps} m/s")
        else:
            # Nếu có V_mps từ người dùng, vẫn cần tính tốc độ khuyến nghị
            try:
                from core.optimize import optimize_speed
                ...
                print(f"DEBUG AUTO SPEED: Using user-provided V_mps={self.p.V_mps} m/s, calculated v_rec={v_rec:.3f} m/s")
